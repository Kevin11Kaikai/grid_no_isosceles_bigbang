#!/usr/bin/env python3
"""Wave3: Type1 defect with cert_lb2 ranking after Type0 2h TIMEOUT."""
...
def main():
    try:
        ...
        pass
    time_s = float(os.environ.get("W3_ORBIT_TIME", "2400"))
    seed = int(os.environ.get("W3_ORBIT_SEED", "821"))
    ...
    cfg = SearchConfig(
        ...
        defect_rank="cert_lb2",
    )
    print(json.dumps({"start": "w3_t1_certlb2", "time_s": time_s, "seed": seed}), flush=True)
    t0 = time.time()
    res = solve_orbit_defect(cfg)
    ck = save_checkpoint(res, f"w3_certlb2_n100_t1_defect_s{seed}")
    ...
    path = os.path.join(exp, f"certlb2_t1_defect_s{seed}.json")
