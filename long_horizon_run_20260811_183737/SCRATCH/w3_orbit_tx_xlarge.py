#!/usr/bin/env python3
"""Wave3: Type3 defect xlarge after Type2."""
...
def main():
    ...
    seed = int(os.environ.get("W3_ORBIT_SEED", "703"))
    workers = int(os.environ.get("W3_ORBIT_WORKERS", "5"))
    sym = int(os.environ.get("W3_ORBIT_TYPE", "3"))
    ...
    print(json.dumps({"start": f"w3_t{sym}_defect_xlarge", "time_s": time_s, "seed": seed}), flush=True)
    t0 = time.time()
    res = solve_orbit_defect(cfg)
    ck = save_checkpoint(res, f"w3_xlarge_n100_t{sym}_defect_s{seed}")
    ...
    path = os.path.join(exp, f"xlarge_t{sym}_defect_s{seed}.json")
