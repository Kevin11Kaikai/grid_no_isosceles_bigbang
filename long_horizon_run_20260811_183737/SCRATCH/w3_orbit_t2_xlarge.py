#!/usr/bin/env python3
"""Wave3: Type2 defect xlarge (Wave2 long TIMEOUT on small U; enlarge now)."""
...
def main():
    ...
    seed = int(os.environ.get("W3_ORBIT_SEED", "702"))
    ...
    print(json.dumps({"start": "w3_t2_defect_xlarge", "time_s": time_s, "seed": seed}), flush=True)
    t0 = time.time()
    res = solve_orbit_defect(cfg)
    ck = save_checkpoint(res, f"w3_xlarge_n100_t2_defect_s{seed}")
    ...
    path = os.path.join(exp, f"xlarge_t2_defect_s{seed}.json")
