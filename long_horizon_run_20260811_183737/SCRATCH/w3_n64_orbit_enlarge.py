#!/usr/bin/env python3
"""Wave3 R1 sandbox: enlarged Type0 defect on n64 (target 113)."""
...
def main():
    try:
        ...
        pass
    time_s = float(os.environ.get("W3_ORBIT_TIME", "1200"))
    seed = int(os.environ.get("W3_ORBIT_SEED", "641"))
    workers = int(os.environ.get("W3_ORBIT_WORKERS", "3"))
    ...
    cfg = SearchConfig(
        ...
        target_size=113,
        defect_budget_min=1,
        defect_budget_max=12,
        time_budget_s=time_s,
        seed=seed,
        num_workers=workers,
        max_extra_orbits=160,
        max_defect_pool=160,
        halo_radius=12,
        agent_c_universe="U_large",
    )
    print(json.dumps({"start": "n64_t0_defect_enlarge", "time_s": time_s, "seed": seed}), flush=True)
    res = solve_orbit_defect(cfg)
    ck = save_checkpoint(res, f"w3_n64_t0_defect_s{seed}")
    ...
    path = os.path.join(exp, f"n64_t0_defect_s{seed}.json")
