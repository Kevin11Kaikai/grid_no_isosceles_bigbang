...
### LH-F081 --- Exact min-cover destroy + all-addable maximize (easiest qs)
- Evidence: `EXPERIMENTS/W3_certcover_max/summary.json` and `summary_n64.json` / `summary_n100.json`.
  - n64: 2 exact-deletion-1 qs; each size-1 cover has **free=2, cap=113**; CP-SAT `SCOPED_INFEAS_TARGET` in <0.1s, `proved_max=112`. Joint both covers: free=4, cap=114, still INFEAS at 113.
  - n100: 16 exact-deletion-2 qs, 24 covers; **every cover has free=3, cap=165**; 10 solved covers all `SCOPED_INFEAS_TARGET` `proved_max=164` (instant). Joint easiest pairs: joint_k=4 (confirms E002), cap=166.
- Level: `SCOPED_INFEASIBLE` (exact on those cores+addable pools).
- Status: **DEAD single-q min-cover frozen-core + individually-addable free** for easiest qs. Does NOT kill Hamming with extra Rem-as-variables, nor non-easiest qs.

### LH-F082 --- Min-cover plus one extra S0 deletion (conflict / geo)
- Evidence: `EXPERIMENTS/W3_cover_plus1/summary.json` + `summary_n64.json` / `summary_n100.json`.
  - n64: 2 covers x 10 extras; all cap still 113; 20/20 `SCOPED_INFEAS_TARGET`.
  - n100: 24 covers; extras from leftover-conflict verts + 8 geo-nearest; **best_cap stays 165** (extra delete opens ~1 new addable, net unchanged); all tried extras `SCOPED_INFEAS_TARGET`. ~250 exact target-INFEAS certificates. `any_plus=false`.
- Level: `SCOPED_INFEASIBLE`.
- Status: **DEAD cover+1 extra frozen-core** on easiest qs. Next: Hamming r=3/4 with Rem=cover\cup extras (F not frozen at |S0|-k).

### LH-F083 --- Hamming r=3/4 cover-halo (reinterpret F081 free-set)
- Evidence: `EXPERIMENTS/W3_cover_hamming_r34/summary.json`. First run ERROR (Add intersected S0). After filtering, 4 symmetry reps: **n_add unselected = 1 or 2**, n_add_from_rem = |Rem|. All r=3/4 `SKIP_SMALL` (need r+1 unselected adds).
- Reinterpretation of F081: the 'free=3' after a min-cover is **{q} ? T** (q plus the two deleted S0 points), not 3 new unselected cells. True unselected surplus = 1.
- Level: `CHEAP_KILL` / cardinality.
- Status: **DEAD this halo Hamming at r=3/4**; r=2 frozen-core cannot net +1 from unselected surplus 1.

### LH-F084 --- Deletion-pair unselected-only gain (205 pairs)
- Evidence: `EXPERIMENTS/W3_pair_unsel_gain/summary.json`. max n_unsel=1; n_unsel_ge3=0; gain<=-1. Easiest covers open exactly their q; top_cert / top_degree / random pairs open **0** unselected cells.
- Level: `FINITE_VERIFIED_SAMPLE` (205 pairs, not all C(164,2)).
- Status: **r=2 frozen-core +1 needs 3 unselected addables; none observed.** Exhaustive for LB=2 qs: see PROVED.md item 6.

### LH-F085 --- k-delete unselected surplus sweep
- Evidence: `EXPERIMENTS/W3_kdelete_unsel/summary.json`. k=2..16 random/top/outer: n_unsel=0. k=24: 0-4; k=32: 0-8. `any_surplus_ge0=false`. Outer still 0 at k=32.
- Level: `FINITE_VERIFIED_SAMPLE`.
- Status: frozen-core delete-k add-all-unsel does not reach +1 in this sweep.

### LH-F086 --- Hamming r=1..7 on top-degree Rem k=32 (Add=8)
- Evidence: `EXPERIMENTS/W3_topdeg32_hamming/summary.json`. Hash `3108292e?5469`. All r=1..7 `INFEASIBLE_SCOPED` in <1s.
- Level: `SCOPED_INFEASIBLE`.
- Status: **DEAD this U_id**. k=24 also INFEAS (F087).

### LH-F087 --- Hamming r=1..3 on top-degree Rem k=24 (Add=4)
- Evidence: `EXPERIMENTS/W3_topdeg24_hamming/summary.json`. Hash `e62c91a3?5218`. Add `[(7,47),(7,52),(92,47),(92,52)]`. All r=1..3 `INFEASIBLE_SCOPED`.
- Level: `SCOPED_INFEASIBLE`.
- Status: **DEAD this U_id**. Same Add family as k=32 subset. Do not rerun top-degree Rem Hamming without a new Add construction.

### LH-F088 --- n64 unselected surplus (exact-1 covers + random k)
