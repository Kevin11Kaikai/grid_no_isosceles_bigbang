...
static void dfs(int depth, u64 *cand){
    ...
    if (dumptarget>0 && depth==dumptarget){
        #pragma omp critical
        { printf("HIT"); for(int i=0;i<depth;i++) printf(" %d,%d",XOF(S[i]),YOF(S[i])); printf("\n"); }
    }
    if (dumptarget<0 && depth > best){
        #pragma omp critical
...
int main(int argc,char**argv){
    ...
    for(int i=4;i<argc;i++){
        ...
        else if(!strcmp(argv[i],"--dump")){ dumptarget=atoi(argv[i+1]); i+=1; }
    ...
    for(int v0=0; v0<N; v0++){
        ...
        for(int v1=v0+1; v1<N; v1++){
            if(usesym){ int ok=1; for(int g=0;g<NG;g++) if(gmap[g][v0]==v0 && gmap[g][v1]<v1){ ok=0; break; }
                        if(!ok) continue; }
